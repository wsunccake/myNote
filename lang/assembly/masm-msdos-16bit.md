...
- `A` (Assemble，組合)
  - 語法： `A` [位址]
  - 用途： 將組合語言指令組譯為機器碼。當您輸入 A 後，DEBUG 會進入組譯模式，您可以一行一行地輸入指令，直到按下 Enter 鍵結束。
- `C` (Compare，比較)
  - 語法： `C` [範圍] [位址]
  - 用途： 比較兩個記憶體區塊的內容。它會列出所有不相符的位址及其值。
- `D` (Dump，傾印)
  - 語法： `D` [位址] [範圍]
  - 用途： 顯示指定記憶體區塊的內容，同時以十六進位和 ASCII 兩種格式呈現。這是最常用來檢視記憶體內容的指令。
- `E` (Enter，輸入)
  - 語法： `E` [位址] [值列表] 或 E [位址] "[字串]"
  - 用途： 直接將資料寫入記憶體中的指定位址。您可以使用十六進位值或用引號括住的 ASCII 字串。
- `F` (Fill，填滿)
  - 語法： `F` [範圍] [值列表]
  - 用途： 用特定的值或位元組序列填滿指定範圍的記憶體。
- `G` (Go，執行)
  - 語法： `G` [位址] [中斷點]
  - 用途： 從指定位址開始執行程式。您也可以設定一或多個中斷點，程式執行到中斷點時會暫停。
- `H` (Hexarithmetic，十六進位運算)
  語法： `H` [值 1] [值 2]
  用途： 執行兩個十六進位數值的加法和減法，並顯示結果。
- `I` (Input，輸入)
  - 語法： `I` [埠位址]
  - 用途： 從指定的 I/O 埠讀取一個位元組。
- `L` (Load，載入)
  - 語法： `L` [位址] [磁碟機] [磁區] [數量]
  - 用途： 從磁碟將檔案或磁區內容載入到記憶體。
- `M` (Move，移動)
  - 語法： `M` [範圍] [目的位址]
  - 用途： 將一個記憶體區塊的內容移動到另一個位址。
- `N` (Name，命名)
  - 語法： `N` [檔案名稱]
  - 用途： 為後續的載入 (L) 或寫入 (W) 指令指定檔案名稱。
- `O` (Output，輸出)
  - 語法： `O` [埠位址] [值]
  - 用途： 將一個位元組的值寫入指定的 I/O 埠。
- `P` (Proceed，跳過)
  - 語法： `P`
  - 用途： 執行單個指令，通常用於跳過子程式呼叫 (CALL) 或迴圈，將其視為一個單一的步驟。
- `Q` (Quit，離開)
  - 語法： `Q`
  - 用途： 結束 DEBUG 程式並返回 MS-DOS 提示符號。
- `R` (Register，暫存器)
  - 語法： `R` [暫存器名稱]
  - 用途： 顯示或修改 CPU 暫存器的內容。如果沒有指定暫存器名稱，則顯示所有暫存器的值和旗標 (flags)。
- `S` (Search，搜尋)
  - 語法： `S` [範圍] [值列表]
  - 用途： 在指定的記憶體範圍內搜尋特定的位元組序列或字串，並列出所有找到的位址。
- `T` (Trace，追蹤)
  - 語法： `T` [數量]
  - 用途： 步進式地執行一個或多個指令，並在每一步後顯示所有暫存器的內容和旗標的狀態。這是最基本的程式偵錯技術。
- `U` (Unassemble，反組譯)
  - 語法： `U` [位址] [範圍]
  - 用途： 將記憶體中的機器碼轉換回組合語言指令，這是 A 指令的反向操作。
- `W` (Write，寫入)
  - 語法： `W` [位址] [磁碟機] [磁區] [數量]
  - 用途： 將記憶體中的內容寫入磁碟上的檔案或磁區。使用此指令前需要先用 N 指令指定檔案名稱。
...
- Load Time:
  ...
  程式從硬碟被載入到記憶體中，準備開始執行的階段。在這個階段，作業系統或載入器會為程式一次性分配所需靜態記憶體。

  - Code/Text Segment, 程式碼 : 存放程式的機器指令。。當程式載入時，通常是唯讀的，以防止程式意外修改指令。
  - Static Data, 靜態資料: 存放全域變數和靜態變數。這些變數的大小是固定的，並且在程式整個執行期間都存在。
    - Data Segment: 程式編譯時賦予初始值的靜態或全域變數。例如：int global_var = 10;。
    - BSS (Block Started by Symbol): 程式編譯時沒有賦予初始值的靜態或全域變數。例如：int global_uninit_var;。BSS 的優點是，執行檔本身不需要儲存這些變數的資料，程式載入時作業系統會自動將這塊區域清零，讓執行檔的體積更小。
  - Stack, 堆疊: 為執行緒分配一個固定大小的堆疊，用於處理函式呼叫、區域變數和返回位址。

- Run Time:

  程式已經開始執行，正在處理指令的階段。程式根據實際需求，動態地向作業系統請求額外的記憶體。

  - Heap, 堆積: 動態記憶體分配的主區域。程式會發出 system call (C 使用 malloc，C++ 使用 new) 向作業系統申請一塊記憶體，並在不再需要時用就釋放 (C 使用 free, C++ 使用 delete)。

```
+------------------+  <-- Low Address
| Code/Text Segment|  <-- 存放程式的機器指令。
+------------------+
|   Data Segment   |  <-- 存放已初始化的全域與靜態變數。
+------------------+
|   BSS Segment    |  <-- 存放未初始化的全域與靜態變數。
+------------------+
|                  |
|      Heap  ↓     |  <-- 存放動態分配的記憶體。
|                  |      生長方向是從低位址往高位址。
+------------------+
|      . . .       |
|  Unused Memory   |  <-- Heap 與 Stack 之間的未使用空間
|      . . .       |
+------------------+
|                  |
|      Stack ↑     |  <-- 存放區域變數與函式參數。
...
code: segment
  ...
  dw 0123h,0456h,0789h,0abch,0defh,0fedh,0cbah,0987h  ; 8 個 word（16 位元）
  dw 0,0,0,0,0,0,0,0
  ; 兩組 8 個 word 的資料，總共佔用 8 * 2 + 8 * 2 = 32 位元組，也就是 20h

start:
  mov ax, cs
  mov ss, ax
  mov sp, 30h
  ; 手動設定堆疊段暫存器 SS 和堆疊指標 SP。
  ; SS (Stack Segment): 決定了堆疊所在的記憶體段。
  ; SP (Stack Pointer): 決定了堆疊段內的偏移位址，也就是堆疊的頂部位置。
